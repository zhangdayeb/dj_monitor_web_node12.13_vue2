...
import { QZNN_BET_MAPPING, STATUS_MAPPING } from '../constants/qznnMapping.js'
...
export function getBetTypeName(betId, isShort = false) {
  const betInfo = QZNN_BET_MAPPING[betId]
...
export function getBetTypeTag(betId) {
  ...
  switch (betInfo.category) {
    case 'basic': return 'primary'    // 基础 - 蓝色
    case 'grab': return 'success'     // 抢庄 - 绿色
    case 'special': return 'warning'  // 特殊 - 橙色
    default: return 'info'
  }
}
  ...
  records.forEach(record => {
    ...
    if (!groups[betId]) {
      groups[betId] = {
        ...
        category: QZNN_BET_MAPPING[betId]?.category || 'unknown',
 ...
 * @returns {Object} 投注建议
 */
export function getQznnAdvice(betTypeStats) {
  ...
  const categories = {
    ...
    grab: { amount: 0, count: 0 },
    special: { amount: 0, count: 0 }
  }
  
  // 统计庄闲投注情况
  const bankerBets = betTypeStats.find(stat => stat.betId === 70) // 庄家
  const playerBets = betTypeStats.filter(stat => [71, 72, 73, 74, 75].includes(stat.betId)) // 5个闲家
  const grabBets = betTypeStats.filter(stat => [76, 77, 78, 79, 80, 81].includes(stat.betId)) // 抢庄
  
  betTypeStats.forEach(stat => {
    const betInfo = QZNN_BET_MAPPING[stat.betId]
  ...
  const playerTotal = playerBets.reduce((sum, bet) => sum + bet.totalAmount, 0)
  const grabTotal = grabBets.reduce((sum, bet) => sum + bet.totalAmount, 0)
  const bankerAmount = bankerBets?.totalAmount || 0
  
  if (total === 0) {
    return { suggestion: '暂无投注数据', trend: 'neutral' }
  }
  
  const basicRatio = (categories.basic.amount / total * 100).toFixed(1)
  const grabRatio = (categories.grab.amount / total * 100).toFixed(1)
  const specialRatio = (categories.special.amount / total * 100).toFixed(1)
  
  let suggestion = `基础${basicRatio}% | 抢庄${grabRatio}% | 特殊${specialRatio}%`
  let trend = 'neutral'
  
  // 分析投注趋势
  if (grabTotal > (bankerAmount + playerTotal) * 0.5) {
    trend = 'grab'
    suggestion += ' - 抢庄热门'
  } else if (bankerAmount > playerTotal * 1.2) {
    trend = 'banker'
    suggestion += ' - 庄家热门'
  } else if (playerTotal > bankerAmount * 1.2) {
    trend = 'player'
    suggestion += ' - 闲家热门'
  } else if (categories.special.amount > categories.basic.amount * 0.3) {
    trend = 'special'
    suggestion += ' - 特殊投注活跃'
  } else {
    suggestion += ' - 投注均衡'
