...
import { BULL_BET_MAPPING, STATUS_MAPPING } from '../constants/bullMapping.js'
...
export function getBetTypeName(betId, isShort = false) {
  const betInfo = BULL_BET_MAPPING[betId]
  ...
  records.forEach(record => {
    ...
    if (!groups[betId]) {
      groups[betId] = {
        ...
        category: BULL_BET_MAPPING[betId]?.category || 'unknown',
 ...
 * @returns {Object} 投注建议
 */
export function getBullAdvice(betTypeStats) {
  ...
  // 统计庄闲投注情况
  const bankerBets = betTypeStats.find(stat => stat.betId === 50) // 庄家
  const playerBets = betTypeStats.filter(stat => [51, 52, 53, 54].includes(stat.betId)) // 4个闲家
  
  betTypeStats.forEach(stat => {
    const betInfo = BULL_BET_MAPPING[stat.betId]
  ...
  let suggestion = `基础${basicRatio}% | 特殊${specialRatio}%`
  let trend = 'neutral'
  
  // 分析庄闲趋势
  if (bankerAmount > playerTotal * 1.2) {
    trend = 'banker'
    suggestion += ' - 庄家热门'
