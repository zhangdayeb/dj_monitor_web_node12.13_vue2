    ...
    <div class="bet-stats-section">
      <div class="section-header">
        ...
        <div class="stats-actions">
          ...
          <!-- 投注趋势建议 -->
          <div class="trend-advice" v-if="baccaratAdvice.suggestion">
            <span class="advice-label">投注趋势:</span>
            <span class="advice-text" :class="`trend-${baccaratAdvice.trend}`">
              {{ baccaratAdvice.suggestion }}
            </span>
          </div>
        </div>
      </div>
...
export default {
  ...
  setup() {
    ...
    // 百家乐投注建议
    const baccaratAdvice = computed(() => {
      const betTypeStats = groupByBetType(records.value)
      return getBaccaratAdvice(betTypeStats)
    })
    ...
    const loadTables = async () => {
      ...
      try {
        const data = await apiService.getBaccaratTables()
        // 适配后端返回的数据格式
        tableList.value = Array.isArray(data) ? data.map(table => ({
          id: table.table_id,
          name: `百家乐台桌${table.table_id}`,
    ...
    const loadRecords = async () => {
      ...
      try {
        const data = await apiService.getBaccaratRecords({ 
    ...
    const loadOverview = async () => {
      ...
      try {
        const data = await apiService.getBaccaratOverview(currentTableId.value)
        // 处理可能为null的返回数据
        overviewData.value = {
    ...
    const loadBetStats = async () => {
      ...
      try {
        const data = await apiService.getBaccaratBetStats(currentTableId.value)
...
.card-icon {
  ...
  border-radius: 50%;
  background: linear-gradient(45deg, #fa8c16, #f56c6c);
}
