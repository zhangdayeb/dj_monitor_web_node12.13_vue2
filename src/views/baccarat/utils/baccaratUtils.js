...
import { BACCARAT_BET_MAPPING, STATUS_MAPPING } from '../constants/baccaratMapping.js'
...
export function getBetTypeName(betId, isShort = false) {
  const betInfo = BACCARAT_BET_MAPPING[betId]
  ...
  records.forEach(record => {
    ...
    if (!groups[betId]) {
      groups[betId] = {
        ...
        category: BACCARAT_BET_MAPPING[betId]?.category || 'unknown',
 ...
 * @returns {Object} 投注建议
 */
export function getBaccaratAdvice(betTypeStats) {
  if (!betTypeStats || betTypeStats.length === 0) {
    return { suggestion: '暂无数据', trend: 'neutral' }
  }
  
  // 找出庄闲投注情况
  const bankerBets = betTypeStats.find(stat => stat.betId === 8) // 庄
  const playerBets = betTypeStats.find(stat => stat.betId === 6) // 闲
  const tieBets = betTypeStats.find(stat => stat.betId === 7) // 和
  
  const bankerAmount = bankerBets?.totalAmount || 0
  const playerAmount = playerBets?.totalAmount || 0
  const tieAmount = tieBets?.totalAmount || 0
  
  const total = bankerAmount + playerAmount + tieAmount
  
  if (total === 0) {
    return { suggestion: '暂无基础投注', trend: 'neutral' }
  }
  
  const bankerRatio = (bankerAmount / total * 100).toFixed(1)
  const playerRatio = (playerAmount / total * 100).toFixed(1)
  const tieRatio = (tieAmount / total * 100).toFixed(1)
  
  let suggestion = `庄${bankerRatio}% | 闲${playerRatio}% | 和${tieRatio}%`
  let trend = 'neutral'
  
  if (bankerAmount > playerAmount * 1.5) {
    trend = 'banker'
    suggestion += ' - 庄家热门'
  } else if (playerAmount > bankerAmount * 1.5) {
    trend = 'player' 
    suggestion += ' - 闲家热门'
  } else if (tieAmount > (bankerAmount + playerAmount) * 0.2) {
    trend = 'tie'
    suggestion += ' - 和局较多'
  }
